    ...
    for (size_t i = 0 ; i < nbKeys ; ++i)
    {
        ...
        if(namedKeyArray[i].id >= 0xA0) {
            ...
            else 
            {
                ...
                //!!// Debug only
                //!!wchar_t _wpryrt[512];
                //!!swprintf_s(_wpryrt, L"PRYRT: v2c=0x%08X => {%02X,%02X,%02X,%02X,%02X,%02X,%02X,%02X} => '%s'", 
                //!!    v2c, 
                ...
                //!!    map_of_vk_oem[namedKeyArray[i].id]);
                //!!OutputDebugStringW(_wpryrt);
            }
            ...
            //!!// Debug Only
            //!!swprintf_s(_tmp_str, L"PRYRT: map i=%d id=%d c=%u => '%hs'", i, namedKeyArray[i].id, MapVirtualKeyA((UINT)namedKeyArray[i].id, MAPVK_VK_TO_CHAR), map_of_vk_oem[namedKeyArray[i].id]);
            //!!OutputDebugStringW(_tmp_str);
        }
    }
    return;
